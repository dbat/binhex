...
typedef enum {tobin, tohex} cvtmode;
...
int showhelp(_TCHAR* arg)
{
	...
	printf(" \n");
	printf(" Version: 0.1.3 build 006\n");
	printf(" Created: 2006.03.14\n");
	printf(" Revised: 2009.11.02\n\n");
	...
	printf(" \tOriginally created to compress huge pi hex data, produces\n");
	printf(" \tsmaller and significantly faster than ordinary packer (zip,7z)\n\n");
	printf(" USAGE:\n");
	printf(" \t%s -b|-h filenames...\n\n", s);
	printf(" ARGUMENTS:\n");
	...
	printf("     (this program took less than 4 sec. while winzip took 2.5 min.\n");
	printf("     7z took 20+ minutes and brought down all CPU cores)\n\n");
	printf("\t%s -b pi_1024M_hex.txt\n\n", s);
	printf("   - Create .bin file from 8G pi hex data:\n");
	printf("     (this program took 27 sec (cached), 7z took it in 2h:49min\n");
...
//const int MS_DAY = 24 * MS_HOUR;      // 86400000
	...
	const _TCHAR* trx[] = { "hex to bin", "bin to hex" };
	const _TCHAR* ext[] = { ".bin", ".hex" };

	if (c < 3) return showhelp(args[0]);

	if (strlen(args[1]) == 2 && args[1][0] == '-')
	switch(args[1][1])
	{
...
tic = clock();
			...
			if (mode == tobin) cvtobin(source, dest, Buf);
			else cvtohex(source, dest, Buf, uppercase);
